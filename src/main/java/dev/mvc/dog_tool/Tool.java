...
public class Tool {
  ...
  public static synchronized String preview(String upDir, String _src, int width,  int height) {
    ...
    // 이미지 파일인지 검사
    if (name.endsWith(".jpg") || name.endsWith(".bmp") || name.endsWith(".png") || name.endsWith(".jpeg")
        || name.endsWith(".gif")) {
      try {
  ...
  public static String getUploadDir() {
    ...
    if (osName.contains("win")) { // Windows
      path = "C:\\kd\\deploy\\dog\\";
      // System.out.println("Windows: " + path);
    } else if (osName.contains("mac")) { // MacOS
      path = "/Users/yourusername/deploy/dog/";
      // System.out.println("MacOS: " + path);
    } else { // Linux
      path = "/home/ubuntu/deploy/dog/";
